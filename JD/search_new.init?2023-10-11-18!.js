define('search_new', ['jdf/1.0.0/unit/search/1.0.0/search', 'jdf/1.0.0/unit/getjsonp/1.0.0/getjsonp', 'product/module/clslog', 'jdf/1.0.0/ui/lazyload/1.0.0/lazyload', './search_plug.js?201710'], function(require, exports, module){
	...
	if ( !SEARCH.ui_js_prefix ) { SEARCH.ui_js_prefix = "//storage.jd.com/retail-mall/search_list/pc/product/search/0.0.1"}
    ...
    var getColorApiUrl = function(functionId,paramJson){
        ...
        var url = colorApi+'?appid=search-pc-java&client=pc&clientVersion=1.0.0&loginType=3&cthr=1'
               +'&functionId='+functionId+'&body='+JSON.stringify(paramJson)
    ...
    function postDraData(functionId, reqParam,data) {
      try {
        if(window.jmfe!=null && data!=null && data.code!=null && data.code<10 && data.echo!=null && functionId!=null){
            window.jmfe.jsagentReport(
              jmfe.JSAGENT_EXCEPTION_TYPE.business,//固定值不变
              751,   // 固定值： 异常码
              "colorApiErr:" + functionId,// 异常信息，带上FunctionId,方便做报警规则
              {
                fid: functionId,
                resp: data.echo,// 只上报code<10(目前有-1,1,2)的数据，上报echo字段
    ...
    var getColorDataAjax = function(functionId,paramJson,callback,errCallback,postParam){
        var url = getColorApiUrl(functionId,paramJson);
        getJsToken(function(res){
            if(res && res.jsToken){
               url = url + "&x-api-eid-token=" + res.jsToken
            }
            $.ajax({
                url: url,
                async: true,
                ...
                success: function(data) {
                    callback(data);
                    postDraData(functionId, paramJson,data)
                },
                error: function(xhr, status) {
                    if(errCallback!=null){
                        errCallback(xhr, status);
                    }else{
                        console.log("接口调用异常:status="+status+","+url);
                    }
                    postDraData(functionId, paramJson,{"code":-1,"echo":"api error:status="+status})
                }
            });
		}, 1000)
    }
    ...
    var getColorDataByUrlSignAjax = function(functionId,paramJson,callback,errCallback,beforeSendCallback,completeCallback){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if(data && data.startsWith("{") ){
                            ...
                            postDraData(functionId, paramJson,resObj)
                        }
                    },
                    beforeSend : function() {
	...
	var build_category_list = function (arr) {
		...
		for(var i=0;i<data.length;i++){
			if (arr.isBrandList){
				...
				url =arr.baseUrl+"&keyword="+arr.key+"&"+arr.finalCid+"="+data[i].Classification+"#J_searchWrap";
			}
			liItem += "<li><a href='"+url+"' title='"
				+data[i].Name+"' onclick='searchlog(1,"+data[i].Classification+","+arr.key+",51)'>"+data[i].Name+"</a></li>";
		}
		return liItem;
	};
	//列表热卖价格
	var get_hotsale_price = function () {
		...
		ids && ids.length > 0 && seajs.use(window.SEARCH.ui_js_prefix + '/js/tools', function(tools){
			tools.getPrice(ids.toString().split(','), pageConfig.price_pdos_off, function (r){
				if ( r ) {
					for (var i=0,j=r.length;i<j;i++){
						var sku = r[i].id.replace('J_', '');
						var price = r[i].p;
						if ( price > 0 ) {
							$("#hotsale .p-price[sku='"+sku+"']").children("strong").html( '\uffe5' + price);
						} else {
							$("#hotsale .p-price[sku='"+sku+"']").children("strong").html( '暂无报价' );
						}
					}
				}
			})
	...
	var getAllQueryParam = function(url){
	    var result = {};
	    if(!url){
	        return result;
	    }
	    ...
	    if(urls!=null && urls.length>0){
            ...
            if ('URLSearchParams' in window) {
                var params = new URLSearchParams(paramUrl);
                for (var [key, value] of params) {
                    result[key] = value;
                }
            }else{
                paramUrl = decodeURIComponent(paramUrl);
                var queryParamsArr = paramUrl.split("&");
			...
			if(!window.adExpoReported) {
				...
				setTimeout(function () {
					if (!window.adExpoReported && $('#hasAdsStore').attr('data-exposal-url') && $('#hasAdsStore').is(':visible')) {
						...
						if (target && target.attr('data-shopid') && target.attr('data-ad-shop-venderid')) {
	...
	window._searchlogList = function(){
		if(event && event.target){
			...
			if(skuId && ulClazz == 'gl-warp clearfix'){
		        ...
		        if ($parent.is('.p-img')) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "1", ii, page, 0, abTestFlag_JL,alg);
		        }else if ($parent.is('.p-name')) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "2", ii, page, 0, abTestFlag_JL,alg);
		        }else if ($curr.is('.addcart')) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "4", ii, page, 0, abTestFlag_JL,alg);
		        }else if ($curr.is('.J_focus') ) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "5", ii, page, 0, abTestFlag_JL,alg);
		        }else if ($curr.is('.J_contrast') ) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "6", ii, page, 0, abTestFlag_JL,alg);
		        }else if ($parent.is('.ebookbuy') ) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "7", ii, page, 0, abTestFlag_JL,alg);
		        }
		        if($curr.attr('id')=='J_comment_'+skuId) {
		            log("search", "list", referrer, thirdCatId, "5", skuId, "3", ii, page, 0, abTestFlag_JL,alg);
		        }
		        if($curr.is('.spu-link')){
                    // 二手入口埋点
                    log("search", "list", referrer, thirdCatId, "5", skuId, "17", ii, page, 0, abTestFlag_JL,alg);
                }
			}
		}
	};
	//埋点
	window.searchlog = getQueryString('forcebot') ? function(){} : ( SEARCH.isList? window._searchlogList: window._searchlogSearch);
	...
	searchUnit.shopFocus = function(shop_id, vender_id, action) {
		if ( action=='do' ) {
			...
			checkLogin(function(){
				...
				getColorDataAjax("pctradesoa_vender_follow",{"sysName":"search.jd.com","venderId":shop_id},function(res) {
				...
				window.seajs.use(['jdf/1.0.0/ui/dialog/1.0.0/dialog'],function(dialog){
					$('body').dialog({
						...
						onSubmit: function(){
							...
							getColorDataAjax("pctradesoa_vender_unfollow",{"sysName":"search.jd.com","venderId":shop_id},function(res) {
								if (typeof res=='object' && res.success){
									var $shopDom = searchGetShopId(shop_id, vender_id);
			...
			getColorDataAjax("pctradesoa_vender_batchIsFollow",{"sysName":"search.jd.com","venderIds":shop_id},function(res) {
	...
	SEARCH.get_shop_info = function() {
		...
		//店铺信息填充
		venderId && getColorDataAjax("pc_search_shop_head",{"ids":venderId},function(res) {
			if ( typeof res=='object' && res.length ) {
				for (var i=0, j=res.length, target, tag, summary; i<j; i++) {
					...
					//京东自营
					if ( res[i].vender_type==1 && venderId !='1000353065' &&  shopId != '1000353065' ) { tag += '<em class="shop-act-tag tag-jd">京东自营</em>'; }
					//店铺标签
					...
					if(res[i].goodShop){
						...
						shophtml +='		<img src="//m.360buyimg.com/cc/jfs/t1/197706/29/27826/4384/63314bc6E21b4c3e9/8b4d5fc44efc5b3f.png" height="16px">';
						shophtml +='	</span>';
						shophtml +='</div>';
						shophtml +='</div>';
	...
	SEARCH.get_digital_price = function(ids, attach) {
        ...
        var param = {}
        param["area"] = area;
	    var skus = ids.replace(/J_/g, '').split(',');
        do {
            var skuList = [];
            var subSkus = skus.splice(0,30);
            for(var i=0;i<subSkus.length;i++){
            skuList.push({"skuId":subSkus[i]});
            }
            param["skuPriceInfoRequestList"] = skuList;
            getColorDataSignAjax("mzhprice_getCustomRealPriceInfoForColor",param,function(data){
            if ( typeof data=='object' ) {
                var r = [];
                if(data && data.skuPriceInfoResponseList!=null){
                    var list = data.skuPriceInfoResponseList
                    for(var i=0;i<list.length;i++){
                        if( list[i].skuId && list[i].priceResult && list[i].priceResult.jdPrice){
                            var id = "J_"+list[i].skuId;
                            var jdPrice = list[i].priceResult.jdPrice;
                            var marketPrice = list[i].priceResult.marketPrice;
                            //r.push({"id":id,"p":jdPrice,"m":marketPrice});
                            var p = "";
                            if ( jdPrice<0 ) {
                                p = '<i>暂无报价</i>';
                            } else if ( jdPrice==0 ) {
                            ...
                            //附属商品
                            if ( attach ) {
                                $("em.J_"+list[i].skuId).html(strip_tags(p));
                            } else {
                                //获取后台返回的价格
                                $("strong.J_"+list[i].skuId).html(p);
                            }
                        }
                    }
                }
	 ...
	 * 调用阿波罗接口
	 */
	SEARCH.get_ware_stock_new = function(ids, tpl, yushouFlag) {
	    if (!this.enable_abl_stock) { return; } // 库存容灾开关
	    var pduid = readCookie('__jda'), pduid = pduid && pduid.indexOf('.')>-1 ? pduid.split('.')[1] : '', pdpin = readCookie('pin') || '', loc_list = (readCookie('ipLoc-djd')||'').replace(/\..*$/, '').split('-'), loc = loc_list.slice(0,4).join('-'), spu_ids = [];
	    //format
        ids = tpl != -1 && ids ? ids.substr(2).split(',J_') : ids;
        //sku库存
        if ( ids ) { get_stock(ids, 30) } else { console.log("ids is null") }
        //库存加载成功时候的回调
		function stock_loaded(json) {
		    if(!json) return;
            var el_btns = $('#J_goodsList').find('.p-operate .p-o-btn[data-stocknew]'), el_btn, wid, info, is_tsyx, data_sv, nostore = [], spu;
            for (var i=0, l=el_btns.length; i<l; i++ ) {
            	el_btn = el_btns.eq(i), wid = el_btn.attr('data-stocknew'), info = json[wid];
            	if(info) el_btn.removeAttr('data-stocknew');
            	if(info && info['stock']==0){
                    if ( tpl=='2') {
                       el_btn.replaceWith('<a class="p-o-btn addcart disabled"><i></i>加入购物车</a>');
                    } else {
                       el_btn.before('<a href="javascript:;" class="'+(tpl=='1' ? 'p-o-btn addcart disabled' : 'disabled')+'"><i></i>加入购物车</a>').remove();
                    }
            	}
            }
            var stock_btns = $('#J_goodsList').find('.p-stock[data-stocknew]')
            for(var i=0, l=stock_btns.length;i<l;i++){
                stock_btn = stock_btns.eq(i),wid = stock_btn.attr('data-stocknew'),provinceName = stock_btn.attr("data-province"),info = json[wid];
            ...
            //价格
            for(var sku in json){
                var el_btns = $('strong.J_'+sku),priceBtn = el_btns.find("i[data-price]");
                priceBtn.removeAttr("data-price");
                var presale = el_btns.attr("data-yushou");
                if(priceBtn.length>0 && json[sku] && json[sku].realPrice){
                    if(json[sku].realPrice && presale!="1"){
                    ...
                    }else if(!!json[sku].realPrice){
                        el_btns.attr("data-price",json[sku].realPrice)
                    }
                    ...
                    if(secondPrice && secondPrice.type>0 && secondPrice.price>0 && secondPrice.title && presale!="1"){
                        ...
                        if(secondPriceCss!=""){
                            ...
                            secondPriceHtml ='<span class="'+secondPriceCss+'" title="'+secondPrice.title+'"><em>￥'+secondPrice.price+'</em><i></i></span>'
            ...
            //预约和预售处理
            if(!!SEARCH.enable_new_yushou){
              try{
                var container = SEARCH.container;
                var slaveWarePresaleContainer = SEARCH.slaveWarePresaleContainer;
                var slaveWarePresaleData = SEARCH.slaveWarePresaleData;
                for(var sku in json) {
                    var data = json[sku];
                    if(data && data.yushou && (data.yushou.type == "1" || data.yushou.type == "2")){
                        var _this = container[sku];
                        var yushou = data.yushou;
						if(_this){
							if(yushou.showPromoPrice && yushou.showPromoPrice == 1 && yushou.sellWhilePresell && yushou.sellWhilePresell == 1 && yushou.type && yushou.type=="1"){
								_this.find('#presaleBelt').show();
								_this.find('#presaleBelt').attr('id','');
							}else{
								SEARCH.presaleShow(_this, yushou, sku) && delete container[sku];
								//计时开关
								open_interval = true;
							}
						}
                        if(slaveWarePresaleContainer[sku]){
							SEARCH.slaveWarePresaleData[sku] = yushou;
							var dataPriceContent = null;
							if ( yushou.type=="1" ) {
								if ( yushou.hidePrice && yushou.hidePrice == 1 ) {
							...
							if(yushou && yushou.showPromoPrice && yushou.showPromoPrice == 1 && yushou.sellWhilePresell && yushou.sellWhilePresell == 1 && yushou.type && yushou.type==1){
				...
				for (var i in container) {
					...
					var realPrice = target.attr('data-price');
                    var priceHtml = "";
                    if(realPrice < 0){
                        priceHtml = "<i>暂无报价</i>"
                    }else if(realPrice == 0){
                        ...
                        priceHtml = "<em>￥</em><i>"+realPrice+"</i>"
                    }
					target.html(priceHtml).removeAttr('data-price');
				}
			  }catch(err){
				console.log(err)
			  }
        ...
        function get_stock(ids, max_length) {
        	do {
                getColorDataSignAjax("pc_search_batch_stock",{"sku":ids.splice(0,max_length).join(','),"yushouFlag":yushouFlag},
                function(data) {
                    stock_loaded(data);
                });
        	} while (ids.length);
	...
	SEARCH.get_ware_stock = function(ids, tpl, callback) {
		...
		function stock_loaded(json, type) {
			...
			if ( type=='sku' ) {
				...
				//商品售止
				//var url = 'coupon.php?type=sale&sku='+nostore.join(',')
				/*nostore.length && $.getJSON(url, function(data){
				...
				var url = getColorApiUrl("pc_search_coupon",{"type":"sale","sku":nostore.join(',')})
                nostore.length && $.ajax({
					url: url,
					async: true,
					dataType: 'jsonp',
	...
	SEARCH.get_icon_info = function() {
		...
		//预约预售
		if ( !this.enable_new_yushou && presale_ids.length) {
			var open_interval = false;
			getColorDataAjax("youshouinfoList",{"sku":presale_ids.join(',')},function(json) {
	...
	SEARCH.get_prompt_adwords = function(adwords_list) {
		var data = null;
		var max_length = 10;
		if(this.enable_prom_adwords && adwords_list && adwords_list.length > 0){
		    do {
                getColorDataAjax("pc_search_adword",{"source":"search_pc","skuids":'['+ adwords_list.splice(0,max_length).join() +']'},function(json) {
                    if (!json && json.skus && json.skus.length > 0){ return; }
	...
	SEARCH.get_comment_nums = function(ids) {
		$.getJSON(getColorApiUrl2("pc_club_productCommentSummaries","referenceIds="+ids.replace(/J_/g, '')+"&callback=?"), function(data){
	...
	SEARCH.get_shop_name = function(tpl){//获取店铺名
		...
		for(var i=0, arr=[], pid; i<shop_length; i++) {
			...
			shop_id && verderId && verderId>0 && arr.push(verderId);
		...
		if ( arr.length ) {
            ...
            getColorDataAjax("getBatchVendersByVenderIds",{"ids":arr.unique().join(","),"country": "cn","businessUnit": "301","language": "zh"},function(res) {
				if( !res ) { return; }
				var map = res;
				for(var i=0; i<shop_length; i++) {
					...
					//图书模板
					tpl=='2' && el_shop.removeAttr('data-shopid').find('a.curr-shop').replaceWith('<a class="curr-shop" target="_blank" onclick="searchlog(1,'+curr.shop_id+',0,58)" href="//mall.jd.com/index-'+curr.shop_id+'.html?from=pc" title="'+curr.shopName+'">'+curr.shopName+'</a>');
					//3C、服装模板
					(tpl=='1' || tpl=='3') && el_shop.removeAttr('data-shopid').html('<span class="J_im_icon"><a target="_blank" onclick="searchlog(1,'+curr.shop_id+',0,58)" href="//mall.jd.com/index-'+curr.shop_id+'.html?from=pc" title="'+curr.shopName+'">'+curr.shopName+'</a></span>');
				}
				//im图标
	...
	SEARCH.get_ware_info = function() {
		var commit_ids = [],main_ids = [], icon_ids=[],adwords_list=[],mainSku_ids = [], plist = $('#J_main'), tpl = plist.find('ul.gl-warp').attr('data-tpl'), attach_ids = [];
		...
		plist.find("div.p-icons").not('[data-done="1"]').each(function(){
			...
			this.setAttribute('data-done', '1');
			if(adwordsSku){
                var adword = '{"skuId" : '+adwordsSku;
                if(spuId && !isNaN(parseFloat(spuId)) && isFinite(spuId)){ adword += ', "spu" : '+spuId };
                if(cid1 && cid2 && cid3){
                    adword += ', "categoryList":['+cid1+','+cid2+','+cid3+']'
                }
                adword += '}'
                icon_ids.push(skuid);
		...
		var mainSkuFlag = [],mainSkuMap = {},container = {};
		var slaveWarePresale_ids = [], slaveWarePresaleContainer = {};
		plist.find("strong[class^='J_']").not('[stock-done="1"]').each(function(){
           this.setAttribute('stock-done', '1');
           mainSku_ids.push(this.className);
           info = $(this).data();
           var yuhsouFlag = "0"
           if(info.yushou=='1' && info.yuyue=='1'){
              yuhsouFlag = "3";
           }else if(info.yushou=='1'){
              yuhsouFlag = "1";
           }else if(info.yuyue=='1'){
           	  yuhsouFlag = "2";
           }
           var sku = this.className.substr(2);
           if(yuhsouFlag!="0"){
               container[sku] = $(this).parent().parent();
           }
           mainSkuFlag.push(yuhsouFlag)
           mainSkuMap[sku] = yuhsouFlag;
        });
        var slaveWarePresaleObj = $('#J_goodsList').find('.ps-main img[data-sku]').not('[yushou-data-done="1"]');
        slaveWarePresaleObj.each(function(){
        	//获取数据
        	var sku = $(this).attr('data-sku'), info = $(this).data();
        	slaveWarePresale_ids.push(sku);
           ...
           if(sku && mainSkuMap[sku]==undefined){
                mainSku_ids.push('J_'+sku);
                mainSkuFlag.push(yuhsouFlag)
                mainSkuMap[sku] = yuhsouFlag;
           }
           this.setAttribute('yushou-data-done', '1');
        });
        SEARCH.slaveWarePresale_ids = slaveWarePresale_ids;
        SEARCH.slaveWarePresaleContainer = slaveWarePresaleContainer;
        SEARCH.container = container;
        //获得阿波罗库存价格
        mainSku_ids.length && this.get_ware_stock_new(mainSku_ids.join(','),tpl,mainSkuFlag.join(''))
		//数字价格
	...
	SEARCH.get_diviner_ware = function() { //少结果调推荐
		...
		var diviner_ware_loaded = function(json) {
				...
				window.seajs.use(_this.ui_js_prefix+'/js/goodsList',function(goodsList){goodsList.init()});
				_this.get_ware_info();
		...
		getColorDataSignAjax("recDivinerApi",{"p":"907006","skus":skus.join(','),"c3":cid,"lid":loc_list[0],"lim":num,"ec":"utf-8"},
        function(data) {
            diviner_ware_loaded(data);
        });
	};
	SEARCH.get_ebook_read = function() { //电子书试读
		var skus = [], _this = this,  plist = $('#J_goodsList');
		//获取页面主商品
		...
		//
		$.ajax({
			url: '//cread.jd.com/readask/canReadForJSONP.action?my=ebook3&bookIds='+skus.join(','),
			dataType: 'jsonp',
			success: function(json) {
				...
				if (list){
					for( var i = 0;i<list.length;i++){
						...
						if (v.isRead){
							//判断a标签个数
							var hasRead = plist.find("li[data-sku="+v.wareId+"]").find("a.freeread");
							if (hasRead.length == 0){
								var item = plist.find("li[data-sku="+v.wareId+"]").find("a.J_contrast");
								if (item.length > 0 ){
									$(item).removeClass("J_contrast contrast").addClass("freeread").attr('href', "javascript:;").html("<i></i>试读");
								}else {
									item = plist.find("li[data-sku="+v.wareId+"]").find("a.J_focus");
									if (item.length > 0 ){
										$(item).removeClass("J_focus focus").addClass("freeread").attr('href',"javascript:;").html("<i></i>试读");
									}else {
										item = plist.find("li[data-sku="+v.wareId+"]").find("a.ebookbuy");
										if (item.length > 0 ){
											$(item).removeClass("ebookbuy").addClass("freeread").attr('href',"javascript:;").html("<i></i>试读");
										}else {
											item = plist.find("li[data-sku="+v.wareId+"]").find("a.addcart");
											if (item.length > 0 ) {
												$(item).removeClass("addcart").addClass("freeread").attr('href', "javascript:;").html("<i></i>试读");
											}
										}
									}
								}

								$(item).find("i").attr('style','background: url(//misc.360buyimg.com/product/list/ebook.1.0.2/css/i/sprite-ebook.png) no-repeat; background-position: -60px -65px;');
								$(item).hover(function () {
									$(item).find("i").css('background-position','-60px -25px');
								},function () {
									$(item).find("i").css('background-position','-60px -65px');
								});
								item.attr('href','//cread.jd.com/read/startRead.action?bookId='+v.wareId+'&readType=1')
    ...
    if(SEARCH.openSearchGoPageColor == "1"){
        //从url中解析参数放入body中,网关参数透传有20个上线
        var param = getAllQueryParam(url);
        param["show_items"] = "";
        param["isList"] = this.isList;
        getColorDataByUrlSignAjax("pc_search_s_new", param || {}, function(res) {
            //异常处理,限流或接口异常
            ...
            if ( ref_type==1 ) {
				...
				anchor_loc();
				//属性区
				window.seajs.use([_this.ui_js_prefix+'/js/selector'], function(selector){selector.init()});
				//重新绑定事件
	...
	SEARCH.success_js = function(ref_type) {
		...
		window.seajs.use(this.ui_js_prefix+'/js/goodsList',function(goodsList){goodsList.init()});
		//换购商品选中,调用UED函数
	...
	SEARCH.page_html = function(i, total_page, result_count, scroll, next_start, prev_start, advware_count, promotion_count, repeat_count) {
		//current page
		this.current_page = i;
		//next search start
	...
	SEARCH.page = function(i, bottom) {
		...
		handle_request(this.base_url+'&isList='+this.isList+'&page='+i+'&s='+start+'&click='+this.click+'&log_id='+LogParm.log_id);
		searchlog(1, i, 0, 56);
	};
	...
	SEARCH.scroll = function() {
		...
		var	url = base+rewrite_fun(this.base_url)+'&page='+page+'&s='+start+'&scrolling=y&log_id='+LogParm.log_id+'&tpl=';
	...
	SEARCH.bind_events = {
		...
		async_category: function(s) {
			...
			var index = base_url && base_url.indexOf("?");
            if ( index>-1 ) { base_url = base_url.substr(index); }
            var index1 = base_url && base_url.indexOf("#");
            if ( index1>-1 ) { base_url = base_url.substr(0, index1); }
            var kv = base_url.split("&")
			if(kv && kv.length>0){
			    for(var i=0;i<kv.length;i++){
			        if(kv[i] && kv[i].indexOf("=")>-1){
			            var kvs = kv[i].split("=")
			            if(kvs && kvs.length>1){
			                param[kvs[0]] = kvs[1]
			            }
			...
			if ( s.c_category ) {
				/*$.get('category.php?'+ filtUrl(base_url, ['ev','psort']) + (s.p_category ? '&c=all' : ''), function(res){
					$('#J_crumbsBar').find('ul[data-level="c"]').append(build_category_list(res)).find('li').length>30 && $('#J_crumbsBar').find('ul[data-level="c"]').parent().addClass('menu-drop-xl');
				})*/
				delete param["ev"];
                delete param["psort"];
                if(s.p_category){
                ...
                getColorDataAjax("pc_search_category",param,function(res) {
                    $('#J_crumbsBar').find('ul[data-level="c"]').append(build_category_list(res)).find('li').length>30 && $('#J_crumbsBar').find('ul[data-level="c"]').parent().addClass('menu-drop-xl');
                });
			}
			if ( s.p_category ) {
				/*$.get('category.php?'+ filtUrl(base_url, ['ev','psort','cid1','cid1','cid3']) + '&cid2=' + s.p_category, function(res){
					$('#J_crumbsBar').find('ul[data-level="p"]').append(build_category_list(res));
				})*/
                delete param["ev"]
                delete param["psort"]
                delete param["cid1"]
                delete param["cid3"]
                param["cid2"] = s.p_category;
                getColorDataAjax("pc_search_category",param,function(res) {
                    $('#J_crumbsBar').find('ul[data-level="p"]').append(build_category_list(res));
                });
			}
	...
	SEARCH.marketLog = function(){
		$('#market').delegate('li', 'click', function (event) {
            ...
            log("search", "list", referrer, thirdCatId, "2", type, href, abTestFlag_JL);
	...
	SEARCH.hotsaleLog = function(){
		 $('#hotsale').delegate('dl', 'click', function(event) {
            ...
            var sku = $(this).find('.p-price').attr('newsku') || $(this).find('.p-price').attr('sku');
            var ind = $(this).index()+1;
            if ($parent.is('.p-name')) {
                log("search", "list", referrer, thirdCatId, "8", "1",sku, "1",ind,  abTestFlag_JL);
            }else if ($parent.is('.btns')) {
                log("search", "list", referrer, thirdCatId, "8", "1", sku, "7", ind,  abTestFlag_JL);
            }else if($parent.is('dt')){
                log("search", "list", referrer, thirdCatId, "8", "1", sku,  "2",ind,  abTestFlag_JL);
            }
        });
	};
	SEARCH.listOpenPageLog = function(){
		...
		var jl = decodeURIComponent(_getAttrValue2( 'JL',location.href));
		var abTestFlag_JL = params.abTestFlag;
        if (jl != 'null' && jl != '') {
            var abTestFlag_JL = params.abTestFlag;
            var jlArr = jl.split("_");
            if (jlArr[0] == 6) {
                log("search", "list", location.href, params.thirdCatId, jlArr[0], params.pageNo, abTestFlag_JL);
            } else if (jlArr[0] == 7) {
                log("search", "list", location.href, params.thirdCatId, jlArr[0], encodeURIComponent(jlArr[1]), abTestFlag_JL);
            } else if (jlArr[0] == 4) {
                log("search", "list", location.href, params.thirdCatId, jlArr[0], encodeURIComponent(jlArr[1]), abTestFlag_JL);
            } else {
                log("search", "list", location.href, params.thirdCatId, jlArr[0], encodeURIComponent(jlArr[1]), encodeURIComponent(jlArr[2]), abTestFlag_JL);
            }
        }
        var skuids1 = [];
        var skuids2 = [];
        ...
        if(skuids1.length>0){
            log("search","list",location.href,params.thirdCatId,0,skuids1.join('-'),abTestFlag_JL, 0);
        }
        if(skuids2.length>0){
            log("search","list",location.href,params.thirdCatId,0,skuids2.join('-'),abTestFlag_JL, 0);
        }
        if(skuids3.length>0){
            log("search","list",location.href,params.thirdCatId,0,skuids3.join('-'),abTestFlag_JL, 0);
        }
	
	};
	SEARCH.init = function(page, total_page, result_count, sort, scroll, recommend, next_start, prev_start, advware_count, promotion_count, repeat_count) {
		recommend && this.get_diviner_ware(),
		this.get_ware_info(),
		this.page_html(page, total_page, result_count, scroll, next_start, prev_start, advware_count, promotion_count, repeat_count),
		this.sort_html(sort),
