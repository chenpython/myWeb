...
import time
import cv2
# import pyautogui  # WSL 缺少 x-server
import requests
...
class HandleCaptcha(object):
    ...
    def sign_in(self):
        try:
            ...
            self.driver.find_element_by_xpath(
                '//input[@title="邮箱"]').send_keys(self.user)
            self.driver.find_element_by_xpath(
                '//input[@title="密码"]').send_keys(self.password)
            time.sleep(random.uniform(2, 4))
    ...
    def slider_verify(self):
        ...
        while not self.check_login():
            ...
            # calculate the scale
            img = cv2.imread(
                './background.png')  # imread函数有两个参数，第一个参数是图片路径，第二个参数表示读取图片的形式
            # 获取实际页面上背景图片的长度
            w1 = img.shape[1]
            ...
            print('distance:{} ,{} times to try'.format(
                distance, try_verify + 1))
            self.slide_by_pyautogui(x=820, y=677,
                                    offset=distance)  # 滑块的坐标与实际偏移量
            return True
    ...
    def slide_by_pyautogui(self, x, y, offset):
        ...
        # pyautogui.moveTo(x + int(offset * random.randint(15, 23) / 20),
        #                  y,
        #                  duration=2.4)
        # y += random.randint(-9, 0)
        # pyautogui.moveTo(x + int(offset * random.randint(17, 21) / 20),
        #                  y,
    ...
    def find_pic(self, target='./background.png', template='./slider.png'):
        try:
            ...
            # 比较位置
            res = cv2.matchTemplate(target_gray, template_rgb,
