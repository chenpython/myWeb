...
class UploadFileHandler:
    ...
    async def download_total_file(self, url, down_headers):
        ...
        file_size = await self.get_file_size(url, down_headers)
    ...
    async def download_resumable(self, url, down_headers, start, end, sem):
        async with sem:
            ...
            content = await self._download(url, down_headers)
            if not content:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        for index in range(
            ...
            if not data:
                break
            executor.submit(item_handler, data)

    end_time = time.perf_counter()
    logger.info('处理item结束，耗时: {:.8f} ……………………'.format(end_time - start_time))
