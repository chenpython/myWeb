...
class WhoClinicSpider(base_spider.CustomBaseSpider):
    ...
    def parse_detail(self, response):
        ...
        # tables = response.xpath('//div[@class="ProjetInfo_ms"]/table')

        item = {}

        # for index, table in enumerate(tables):

        #     method = getattr(self, 'table_{}_handler'.format(index + 1))

        #     if not method:
        #         self.logger.error("missing method, table content: {}".format(
        #             table.get()))
        #         continue
        ...
        content = response.xpath(
            '//div[@class="index_part2"]/following-sibling::div[1]').get()

        info = response.meta['info']

        href = response.xpath('//a[contains(text(), "下载XML文档")]/@href').get()

        attachments = [{
            'file_url':
            'http://{}/{}'.format(self.allowed_domains[0], href),
            'filename':
            '{}.xml'.format(self.amend_filename(title)),
            'ori_url':
            re.search(r"<a.*href=\"(.*?)\" .*下载XML文档.*<\/a>", content).group(1)
        }]
    ...
    def save_response(self, response):
        ...
        dowloads_folder = '/root/crawlab_workspace/who_clinic'

        filename = self.amend_filename(response.url[-30:-1])
